...
class MultiEmbodimentActionEncoder(nn.Module):
    ...
    def forward(self, actions, timesteps, cat_ids):
        ...
        Args:
            ...
            timesteps: [B,] shared timesteps or [B, T] per-action timesteps
            cat_ids: [B,] category/embodiment IDs
        Returns:
        ...
        # Expand a shared flow timestep across the action horizon. Training-time
        # RTC passes (B, T) directly so clean prefix tokens can use the terminal
        # flow timestep while noisy postfix tokens use the sampled timestep.
        if timesteps.dim() == 1 and timesteps.shape[0] == B:
            ...
            raise ValueError(
                "Expected `timesteps` to have shape "
                f"({B},) or ({B}, {T}), got {tuple(timesteps.shape)}."
            )
